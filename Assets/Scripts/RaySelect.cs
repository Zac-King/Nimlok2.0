...
public class RaySelect : MonoBehaviour
{
    ...
    {
        if (Physics.Raycast(transform.position, transform.forward, out rayCursor, 10000))  // TeleportNode layer
        {
            if (!loader.isLoading &&    // makes sure not to trigger a load if one is not  already in progress
                rayCursor.collider.gameObject.GetComponent<RaySelectionVolume>() != null)  
            {
                OnSelectionHit();   // RayCast on a valid Selection Volume
            }
            else if(rayCursor.collider.gameObject.GetComponent<RaySelectionVolume>() == null)
            {
                OnSelectionExit();
            }
        }
